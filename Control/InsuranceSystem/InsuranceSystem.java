...
public class InsuranceSystem {
	...
	public static void main(String[] args) throws ParseException {
		...
		while(check) {
			...
			System.out.println("1.상품 설계하기, 2.인수정책수립하기, 3.영업활동, 4.인수심사하기, 5.계약 관리하기, 6.보험처리하기");
			int menuNum = 1;
			while(true) {
			...
			case 4:
				//인수 승인 대기 신청서가 없는 경우
				if(subscriptionDAO.showAcceptanceAprove().size() == 0) {
				...
				int no = 0;
				for(no=0; no<subscriptionDAO.showAcceptanceAprove().size(); no++) {
					if(no%2 == 0) {
						System.out.print(no/2+1);
						System.out.print(".("+"보험ID:" + subscriptionDAO.showAcceptanceAprove().get(no));
					}else {
						System.out.println(" 고객ID:" + subscriptionDAO.showAcceptanceAprove().get(no) + ")");
					}
				...
				insuranceSubscription.Accept(subscriptionDAO.showAcceptanceAprove().get(index),subscriptionDAO.showAcceptanceAprove().get(index+1), scanner);
				break;
			case 6:
				...
				case 1:
					...
					//인수 승인 대기 신청서가 없는 경우
					if(subscriptionDAO.showSubscriptionCustomer().size() == 0) {
						System.out.println("인수 승인 대기 신청서가 없습니다.");
						break;
					}
					...
					//no = showacceptanceapprove에서 받은 백터의 index
					no = 0;
					for(no=0; no<subscriptionDAO.showSubscriptionCustomer().size(); no++) {
						if(no%2 == 0) {
							System.out.print(no/2+1);
							System.out.print(".("+"보험ID:" + subscriptionDAO.showSubscriptionCustomer().get(no));
						}else {
							System.out.println(" 고객ID:" + subscriptionDAO.showSubscriptionCustomer().get(no) + ")");
						}
					...
					index = (num *2) - 2;
					insuranceTreatment.AccidentReception(subscriptionDAO.showSubscriptionCustomer().get(index+1),subscriptionDAO.showSubscriptionCustomer().get(index), scanner);
					break;
				case 2:
					//사고가 없는 경우
